...
export class SupabaseTaskRepository {
  async list(userId:string,groupId:string){const {data,error}=await client().from("personal_tasks").select("*").eq("user_id",userId).eq("group_id",groupId).order("created_at");if(error)throw error;return (data as TaskRow[]).map(map)}
